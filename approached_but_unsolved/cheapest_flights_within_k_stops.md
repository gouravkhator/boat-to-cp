...
class CheapestFlightsKStops {
  ...
  Test cases: (in format of n, then flights, then src, then dst, then k)
  ...
  1
  5
  [[0,1,100],[1,2,100],[1,3,600],[2,3,200],[2,4,500],[3,4,100]]
  1
  4
  2
  4
  ...
  */
  ...
  // Problem Question: https://leetcode.com/problems/cheapest-flights-within-k-stops/
  public int findCheapestPrice(int n, int[][] flights, int src, int dst, int k) {
    int currentK = 0;

    Queue<City> parents = new LinkedList<>();
    Queue<City> children = new LinkedList<>();
    int resDist = Integer.MAX_VALUE;

    parents.add(new City(src, 0));

    while (!parents.isEmpty()) {
      City node = parents.poll();
      int cityInd = node.index;
      int cityDist = node.distance;
      ...
      for (int i = 0; i < flights.length; i++) {
        ...
        if (flight[0] == cityInd) {
          ...
          if (flight[1] == dst) {
            resDist = Math.min(resDist, cityDist + flight[2]);
          }

          children.add(new City(flight[1], cityDist + flight[2]));
        }
      } // end of for loop

      if (parents.isEmpty()) {
        if (currentK == k) {
          // added all the children and if now the current k is equal to k, we just return that, before incrementing currentK
